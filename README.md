...
- **Lançar uma despesa em três campos** — o formulário abre a ecrã cheio no telemóvel com o essencial e nada mais: **descrição, valor, data** e **«Registar»**. Os atalhos de data mostram-na («Hoje, 25/ago», «Ontem, 24/ago») e o seletor de outra data só aparece quando se pede. A despesa fica em nome de quem a lança e divide-se pelo normal do grupo — e o cartão mostra logo o **resumo do que vai ser gravado** (quem pagou, divisão, categoria sugerida pela descrição e quanto fica cada pessoa), por isso não há nada a adivinhar; tocar numa linha do resumo abre o assistente já no passo onde se muda.
- **«Mais detalhe» quando é preciso** — o segundo botão do cartão abre um **assistente de 5 passos** (despesa → categoria → quem pagou → divisão → rever), uma pergunta por ecrã — é aqui que se liga a **repetição mensal**, com alvos de toque grandes: grelha de categorias com todas à vista, pessoas em cartões com avatar e o valor de cada uma ao lado. A barra de progresso no topo salta para qualquer passo e o **resumo final** repete tudo o que vai ser gravado — cada linha leva de volta ao passo respetivo. Ao abrir uma despesa já lançada entra-se logo pelo resumo.
...
- **Fatura repartida por várias categorias** — uma despesa pode alocar o valor por **várias categorias** (ex.: 50 € de jantar = 35 € Restaurante + 10 € Bebidas + 5 € Sobremesas). No passo da categoria, «Repartir por várias categorias» abre a alocação por categoria (com «Distribuir igualmente»); a soma tem de bater no total. Os totais por categoria (chips, filtros e relatório) contam **cada parte na sua categoria**, e a lista assinala a fatura repartida com um contador no ícone.
- **Divisão por categoria** — numa fatura repartida podes ainda dizer **por que pessoas se divide o custo de cada categoria** (no passo «Divisão», liga «Dividir cada categoria por pessoas diferentes»). Cada categoria divide-se em **partes iguais** por quem participa nela — ex.: o vinho só entre os adultos, a sobremesa só para quem comeu. O que cada pessoa deve é a **soma** das suas partes; saldos, acertos e relatório continuam a bater certo.
